...
import dev.stas.weatherapp.presentation.details.DetailStore
import dev.stas.weatherapp.presentation.search.SearchStore.Intent
import dev.stas.weatherapp.presentation.search.SearchStore.Label
import dev.stas.weatherapp.presentation.search.SearchStore.State
        ...
        override fun executeIntent(intent: Intent, getState: () -> State) {
            when(intent){
                is Intent.ChangeSearchQuery -> {
                    dispatch(message = Msg.ChangeSearchQuery(intent.query))
                }
                Intent.ClickBack -> {
                    ...
                    when(openReason){
                        OpenReason.AddToFavorite -> {
                            scope.launch {
                                ...
                                publish(label = Label.SavedToFavorite)
                            }
                        }
